	...
	void consulterCommande() {
	    	...
	    	try {
	    		...
	    		for (int i = 0; i < tail_com ; i++) {
	    			...
	    			if (com.getIdClient().equals(idClient)) {
	    		...
	    		if (tail > 0) {
	    			...
	    			for (int j = 1; j <= tail; j++) {
		    			...
		    			System.out.println("\n---> Commande n°"+j+" | soldé : "+ pj.getetatSolde() + "  |livree : "+ pj.getLivree()+"\n");

	    				float som = 0;

	    				
		    			for (int p = 0; p < idProduitJ.size(); p++) {
	    					//System.out.println(idProduitJ.get(p));

		    				int test = 1;
		    				int cmp = 0;
		    				
		    				while ((test == 1) & (cmp < Produit.getNbProduit())) {
		    					
		    					Produit prd = Produit.getProduit(cmp);

		    					//System.out.println(prd.getIdProduit());
		    					//System.out.println(idProduitJ.get(p));
	    						//test = 0;

		    					if (idProduitJ.get(p).equals(prd.getIdProduit())) {
		    						
		    						float prix = prd.getPrix();
		    						som += prix;
		    						
		    						System.out.println("     "+(p+1)+")"+prd.getNom()+"  |  "+prix+"euros");
		    			...
		    			System.out.println("\n   TOTAL COMMANDE = "+som+"euros");
	    			}
	    			
	    			System.out.println("\n\nVoulez vous validez une commande : \n--> oui : 1\n---> non : 2");
	    			...
	    			if(choix == 1) {
    					...
    					while((o < tail_com) & (t == 1)) {
    						...
    						Commande cm = ListCommande.get(choice-1);
    						
    						if(cm.getIdCommande().equals(Commande.getCommande(o).getIdCommande())) {
    							
    							cm.setEtatSolde(true);
    							Commande.remplacer(o, cm);    /*On fait passer la commande en payer*/
    							t = 0;
    						}
    						o++;
    					}
	    		...
	    		System.out.println("oups petite erreur, reesayez !");	
	    	}	    	
	    }
    		...
    		while(veutCommander == 1) {  		
	    		...
	    		for (int i = 0; i < Produit.getNbProduit(); i++) {
	    			...
	    			if ((p.getNom().equals(nom)) && (p.getVendu() == false) ) {
	    				tabPrd.add(p);
	    				p.setVendu(true);;
	    				Produit.remplacer(i, p);			/* mettre le produit en vendu*/
	    			}
	    		...
	    		if (tabPrd.size() > 0) {
		    		...
		    		while((q < Produit.getNbProduit()) && (disponible == 1)) {
		    		...
		    		System.out.println("\n---> Aucun produit ne correspond à ce nom ! ");
	    		}
